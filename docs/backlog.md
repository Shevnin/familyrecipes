...
38. ~~`REQ-09` Добавить поле `история рецепта` в create-request flow.~~ [done — iOS + Edge Function]
39. ~~`DETAIL-10` Показать `история рецепта` отдельным блоком над основным текстом рецепта.~~ [done — iOS]
...
42. ~~`BE-IMPL-04` Реализация unified recipe card read-model + delete + `recipe_story` в Supabase/Edge Functions/read queries.~~ [done]
